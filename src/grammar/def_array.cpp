...
template <typename I> void generic_init_def_array(rule_container<I> & rules)
{
	...
	rules.def_array =
		qi::eps
		[
			_val = create_def_node_sptr(def::node::kind::array)
		]
		>>
		rules.def_type_instanciation_no_array
		[
			/* Initializes the array data */
			_a = construct<def::array_data_t>(),
			/* Sets the type instanciation */
			boost::phoenix::bind(&def::array_data_t::type_instanciation, _a)
				= _1
		]
		>>
		+
		(
			...
			(
				qi::int_
				[
					boost::phoenix::bind(&def::array_data_t::add_dim, _a, _1)
				]
				|
				qi::eps
					[ boost::phoenix::bind(&def::array_data_t::add_dim, _a,
					boost::phoenix::val(0)) ]
			)
			>>
			qi::char_(']')
		...
		qi::eps
		[
			boost::phoenix::bind(&def::node::set_array_data, *_val, _a)
		];
}
