...
template <typename I> void generic_init_def_member(rule_container<I> & rules)
{
	...
	rules.def_member =
		...
		qi::eps
		[
			_val = create_def_node_sptr(def::node::kind::member)
		]
		>>
		-(
			qi::string("optional")
			[
				boost::phoenix::bind(
					&def::node::add_qualifier, 
					*_val,
					def::node::qualifier::optional
				)
			]
        )
		>>
		...
		[
			...
			_a = construct<def::member_data_t>(),
			/* Sets the type instanciation returned by the parser */
			boost::phoenix::bind(&def::member_data_t::type_instanciation, _a)
				= _1
		]
		>>
		rules.identifier
		...
		>> -(
			...
			rules.dat_value
			[
				boost::phoenix::bind(&def::member_data_t::default_value_node,
					_a) =_1
			]
		)
		>>
		...
		[
			...
			boost::phoenix::bind(&def::node::set_member_data, *_val, _a)
		];
}
